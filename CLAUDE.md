...
## اقتصاد التوكنز — التزم بها بكل مهمة بهذا المستودع

- **لا تقرأ ملفات كاملة إذا كان جزء منها يكفي**: استخدم `Grep`/`offset`+`limit`
  بـ`Read` بدل تحميل ملف ضخم كامل (خصوصاً `watch_screen.dart`/
  `watch_screen_discovery.dart` وTECHNICAL.md — كلها آلاف الأسطر).
- **لا تُعِد قراءة ملف قرأته بالفعل بهذي المحادثة** إلا إذا عدّلته أنت
  خارج أداة `Edit` (التي تتحقق من التعديل تلقائياً) أو تغيّر على القرص من
  مصدر خارجي.
- **لا تستخدم Agent الفرعي (`Agent` tool) لمهمة تقدر تنجزها مباشرة** بقراءة/
  بحث محدود — فرع Agent يعيد استنتاج كل السياق من الصفر ويكلّف أكثر بكثير
  من أداة مباشرة.
- **اجمع نداءات الأدوات المستقلة بنفس الرد** (قراءة عدة ملفات، أو بحث +
  قراءة) بدل نداء واحد بكل مرة والانتظار.
- **ردودك النصية للمستخدم مختصرة ومباشرة** — بلا تكرار للسياق المعروف له
  أصلاً، بلا سرد خيارات لن تُنفَّذ، بلا مقدمات/خواتيم زائدة. اذكر الخلاصة
  والقرار، لا رحلة التفكير الكاملة.
- **لا تشغّل `flutter analyze`/`flutter test` أو أوامر بناء كاملة كتحقّق
  استكشافي عشوائي** — فقط بعد تعديل فعلي على الكود، وبالهدف الأضيق الممكن
  (اختبار واحد بدل كل السويت إذا كفى للتحقق من التغيير المطلوب).
- هذي القاعدة إضافة اقتصادية بحتة — **لا تتجاوز** بها القواعد الأخرى بهذا
  الملف (تشخيص السجل الحقيقي قبل `watch_screen*`, قواعد الحماية، إلخ) —
  دقة التشخيص/الأمان أولاً، والاقتصاد بكيفية تنفيذها لا بتفريغ محتواها.
