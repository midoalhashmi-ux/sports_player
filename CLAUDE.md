...
| `Compilation error` بمهمة `:share_plus:compileReleaseKotlin` | إصدار 10.1.4 (المحلول من `^10.0.2`) غير متوافق مع Kotlin 2.2.20/AGP حديثين — نفس فئة خلل `image_gallery_saver_plus` أعلاه بالضبط | `pubspec.yaml`: `share_plus: ^13.3.0` (واجهة `Share.share`/`shareXFiles` القديمة لا تزال تعمل، deprecated فقط) **و**`android/settings.gradle`: AGP من `8.11.1` لـ`8.13.0` (share_plus 12.0.0+ يتطلب AGP >=8.12.1 صراحة) — راجع `TECHNICAL.md` #46 |
